...
#'                  the phenotype file.
#' @param family the type of the phenotype: "gaussian", "binomial". If not provided or NULL,
...
#' @export
snpnet_offset <- function(genotype.pfile, phenotype.file, phenotype, family = NULL, covariates = NULL,
  ...
  time.start <- Sys.time()
  snpnet:::snpnetLogger('Start snpnet', log.time = time.start)
  ...
  ### --- Read phenotype file --- ###
  phe[['master']] <- readPheMaster_offset(phenotype.file, ids[['psam']], family, covariates, offset, phenotype, status.col, split.col, configs)
  ...
  for(s in splits){
    ...
    if (length(offset) > 0) {
      offsets[[s]] <- phe[[s]][, offset, with = F]
    } else {
      offsets[[s]] <- NULL
    }
    if(configs[['verbose']]) snpnet:::snpnetLogger(sprintf("The number of individuals in %s set: %d", s, dim(phe[[s]])[1]))
  }
  ...
  ### --- End --- ###
  snpnet:::snpnetLoggerTimeDiff("Preprocessing end.", time.start, indent=1)
  
  if (configs[['prevIter']] == 0) {
    snpnet:::snpnetLogger("Iteration 0")
    if (family == "cox"){
      return(NULL)
    } else {
      glmmod <- stats::glm(
        stats::as.formula(paste(phenotype, " ~ ", paste(c(1, covariates), collapse = " + "))),
        data = phe[['train']], family = family,  offset = unlist(offsets[['train']])
      )
    ...
    if (configs[['verbose']]) snpnet:::snpnetLogger("  Start computing inner product for initialization ...")
    time.prod.init.start <- Sys.time()
    
    prod.full <- snpnet:::computeProduct(residual, genotype.pfile, vars, stats, configs, iter=0) / nrow(phe[['train']])
    ...
    if (configs[['verbose']]) snpnet:::snpnetLoggerTimeDiff("  End computing inner product for initialization.", time.prod.init.start)
    ...
    if (is.null(lambda)) {
      ...
      full.lams <- snpnet:::computeLambdas(score, configs[['nlambda']], configs[['lambda.min.ratio']])
    } else {
    ...
    snpnet:::snpnetLogger(paste0("Recover iteration ", configs[['prevIter']]))
    ...
    snpnet:::snpnetLoggerTimeDiff("Time elapsed on loading back features", time.load.start)
    earlyStopNow <- (validation && snpnet:::checkEarlyStopping(metric.val, max.valid.idx, configs[['prevIter']], configs))
  }
  cat("\n")
  ...
  if(! earlyStopNow){
    for (iter in (configs[['prevIter']]+1):configs[['niter']]) {
      ...
      if (configs[['verbose']]) snpnet:::snpnetLogger("Start updating feature matrix ...", indent=1)
      ...
      if (configs[['verbose']]) snpnet:::snpnetLoggerTimeDiff("End updating feature matrix.", time.update.start, indent=2)
      if (configs[['verbose']]) {
      ...
      if (configs[['verbose']]){
        if(configs[['use.glmnetPlus']]){
          snpnet:::snpnetLogger("Start fitting Glmnet with glmnetPlus ...", indent=1)
        }else{
          snpnet:::snpnetLogger("Start fitting Glmnet ...", indent=1)
        }
      }
      if (is.null(p.factor)){
      ...
      if (configs[['use.glmnetPlus']]) {
        ...
        if(family == "cox"){
          ...
          glmfit <- glmnetPlus::glmnet(
            features[['train']], response[['train']], family = family, alpha = alpha, offset = unlist(offsets[['train']]), 
          ...
          if(family=="gaussian"){
            ...
            pred.train <- stats::predict(glmfit, newx = as.matrix(features[['train']]), newoffset = unlist(offsets[['train']]), type = "response")
        ...
        if(family=="cox"){
          ...
          glmfit <- glmnet::glmnet(
            tmp.features.matrix, response[['train']], family = family, alpha = alpha, offset = unlist(offsets[['train']]), 
            lambda = current.lams.adjusted[start.lams:num.lams], penalty.factor = penalty.factor,
            standardize = configs[['standardize.variant']], thresh = configs[['glmnet.thresh']],
            type.gaussian = "naive"
          )
          pred.train <- stats::predict(glmfit, newx = tmp.features.matrix, type = "response", newoffset = unlist(offsets[['train']]))
          residual <- response[['train']] - pred.train
        }
      ...
      if (configs[['verbose']]) snpnet:::snpnetLoggerTimeDiff("End fitting Glmnet.", time.glmnet.start, indent=2)
      
      ### --- KKT Check --- ###
      if (configs[['verbose']]) snpnet:::snpnetLogger("Start checking KKT condition ...", indent=1)
      time.KKT.start <- Sys.time()
      
      check.obj <- snpnet:::KKT.check(
      ...
      if ( prev.max.valid.idx == max.valid.idx ) {
        ...
        if (validation) {
          ...
          if (family == "cox") {
            ...
            pred.val <- glmnetPlus::predict.glmnet(glmfit, newx = as.matrix(features[['val']]), lambda = current.lams.adjusted[start.lams:max.valid.idx], newoffset = unlist(offsets[['val']]), type = "response")
          } else {
            pred.val <- glmnet::predict.glmnet(glmfit, newx = as.matrix(features[['val']]), lambda = current.lams.adjusted[start.lams:max.valid.idx], newoffset = unlist(offsets[['val']]), type = "response")
          }
          snpnet:::snpnetLoggerTimeDiff("Time of prediction on validation matrix", time.val.pred.start, indent=2)
        }
        ...
        if (family == "cox") {
          ...
          metric.train[start.lams:max.valid.idx] <- snpnet:::computeMetric(pred.train[, 1:check.obj[["max.valid.idx"]], drop = F], response[['train']], configs[['metric']])
          if (validation){
            snpnet:::snpnetLogger('metric val.')
            metric.val[start.lams:max.valid.idx] <- snpnet:::computeMetric(pred.val, response[['val']], configs[['metric']])
          }
        }
      ...
      if (configs[['verbose']]) snpnet:::snpnetLoggerTimeDiff("End checking KKT condition.", time.KKT.start, indent=2)
      ...
      if ( prev.max.valid.idx < max.valid.idx ) {
        ...
        if (validation) {
          snpnet:::snpnetLogger('Training and validation metric:', indent=1)
        }else{
          snpnet:::snpnetLogger('Training metric:', indent=1)
        }
        for (klam in (prev.max.valid.idx+1):max.valid.idx) {
          if (validation) {
            snpnet:::snpnetLogger(paste0("- Lambda idx ", klam, ". Training: ", metric.train[klam], ". Validation: ", metric.val[klam]), indent=1)
          } else {
            snpnet:::snpnetLogger(paste0("- Lambda idx ", klam, ". Training: ", metric.train[klam], ". "), indent=1)
          }
        }
        prev.max.valid.idx <- max.valid.idx
      }
      ...
      if (validation && snpnet:::checkEarlyStopping(metric.val, max.valid.idx, iter, configs)) break
    }
  }
...
## modified snpnet:::readPheMaster function that can read a offset column in the phenofile
...
#'                   fitting, whose coefficients will not be penalized. The names must exist in the
#'                   column names of the phenotype file.
#' @param offset offset a character vector containing the names of the offset included in the lasso
#'               fitting. The names must exist in the column names of the phenotype file.
#' @param phenotype the name of the phenotype. Must be the same as the corresponding column name in
#'                  the phenotype file.
...
#' @importFrom magrittr %>%
#' @importFrom dplyr n
readPheMaster_offset <- function(phenotype.file, psam.ids, family, covariates, offset = NULL, phenotype, status, split.col, configs){
    if(!is.null(family) && family == 'cox'){
        return(NULL)
    } else{
        selectCols <- c("FID", "IID", covariates, offset, phenotype, split.col)
    }

    phe.master.unsorted <- data.table::fread(
      cmd=paste(snpnet:::cat_or_zcat(phenotype.file, configs), phenotype.file, ' | sed -e "s/^#//g"'),
      # colClasses = c("FID" = "character", "IID" = "character", setNames(rep('numeric', length(phenotype)), phenotype), setNames(rep('numeric', length(offset)), offset)),
      select = selectCols
    )
